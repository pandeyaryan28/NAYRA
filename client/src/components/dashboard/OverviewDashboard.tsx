...
export const OverviewDashboard: React.FC = () => {
  ...
  return (
    <div className="p-6 sm:p-8 max-w-6xl mx-auto space-y-6 transition-colors duration-150">
      {/* Top Greeting & Workstation HUD */}
      <div className="pb-4 border-b border-slate-200 dark:border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-zinc-100">
              {briefing?.greeting || 'Executive Cockpit'}, Commander Aryan
            </h2>
            <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 border border-slate-200 dark:border-zinc-700 font-medium">
              OPERATIONAL
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <div className="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-xs font-mono text-slate-600 dark:text-zinc-400 flex items-center gap-1.5 shadow-2xs">
            <Clock className="w-3.5 h-3.5 text-slate-400 dark:text-zinc-500" />
            <span>IST Workstation</span>
          </div>
        </div>
      </div>

      {/* 4 Focused High-Density Metric Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Tasks Card */}
        <div 
          onClick={() => navigate('/tasks')}
          className="p-4 rounded-xl bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-700 transition-all cursor-pointer space-y-2 shadow-2xs group"
        >
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400 dark:text-zinc-500 font-mono">
            <span>{urgentTasks.length > 0 ? `${urgentTasks.length} high priority` : 'Clean queue'}</span>
        ...
        >
          ...
          <div className="flex items-center justify-between text-[11px] text-amber-500 font-mono">
        ...
        >
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400 dark:text-zinc-500 font-mono">
            <span>{focusMinutesToday > 0 ? `${Math.round(focusMinutesToday / 25)} blocks` : 'Ready to start'}</span>
            <span className="text-violet-600 dark:text-violet-400 opacity-0 group-hover:opacity-100 transition-opacity">→</span>
          </div>
        </div>
        ...
        >
          ...
          <div className="w-full bg-slate-100 dark:bg-zinc-800 rounded-xs h-1.5 overflow-hidden">
            <div className="bg-slate-900 dark:bg-zinc-200 h-full rounded-xs transition-all duration-300" style={{ width: `${caloriePercent}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 rounded-xl bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 shadow-2xs space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => navigate('/tasks')}
              className="text-xs text-slate-500 hover:text-slate-900 dark:hover:text-zinc-100 flex items-center gap-1 cursor-pointer font-mono"
            >
              <span>View all</span>
          ...
          <div className="p-5 rounded-xl bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 shadow-2xs space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-md bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 font-medium">
