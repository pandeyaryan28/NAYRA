...
export const TaskManager: React.FC = () => {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-6 transition-colors duration-150">
      ...
      {/* Quick Add Bar */}
      <form onSubmit={handleQuickAdd} className="flex items-center gap-2 p-2 rounded-xl bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 shadow-2xs">
        ...
        <select
          ...
          className="text-xs bg-slate-50 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-700 rounded-lg px-2 py-1 focus:outline-none cursor-pointer"
        >
          ...
          {filteredTasks.map(task => (
            ...
            >
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`w-4 h-4 rounded border flex items-center justify-center transition-colors cursor-pointer ${
                    task.status === 'completed'
                      ...
                      : 'border-slate-300 dark:border-zinc-700 hover:border-slate-900 dark:hover:border-zinc-300 text-transparent'
                  }`}
                >
                ...
                <div>
                  <span className={`text-xs font-medium ${task.status === 'completed' ? 'line-through text-slate-400 dark:text-zinc-500' : 'text-slate-900 dark:text-zinc-100'}`}>
                    {task.title}
                  </span>
                  {task.notes && (
